...
/**************************************************
 * ASPMDateTime
 * Serves as a date & time helper for the application
 *
...
class ASPMDateTime {
	...
	/*********************************************
	 * Converts db formatted time to app / more user
	 * readable format.
	 *
	...
	public static function ValidDBDate($d = null) {
		if(!isset($d) || is_null($d)) return false;
		if(strlen($d) != 10 || strlen(str_replace("-", "", $d)) != 8 || strlen((string)((double)str_replace("-", "", $d))) != 8) return false;
		return true;
	}
